...
def compute_carbon_emission(journey_distance, vehicle_efficiency, emission_factor):
    if vehicle_efficiency <= 0:
        raise ValueError("Vehicle efficiency must be greater than zero.")
    liters_consumed = journey_distance / vehicle_efficiency
    carbon_kg = liters_consumed * emission_factor
...
def run_carbon_app():
    ...
    try:
        ...
        current_emission_factor = fueltype.get_emission_factor_for_fuel(user_fuel_choice_num)
